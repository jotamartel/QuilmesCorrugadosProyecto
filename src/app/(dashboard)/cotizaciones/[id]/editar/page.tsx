...
import { ArrowLeft, Plus, Trash2, Calculator, AlertTriangle, Save, Upload, X, FileText } from 'lucide-react';
import { formatCurrency, formatM2 } from '@/lib/utils/pricing';
import { formatBoxDimensions } from '@/lib/utils/format';
import type { Client, Box, CalculateQuoteResponse, CalculatedItem, QuoteItem as DBQuoteItem } from '@/lib/types/database';
...
export default function EditarCotizacionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [existingDesign, setExistingDesign] = useState<{ name: string; file_url: string } | null>(null);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [quoteRes, clientsRes, boxesRes] = await Promise.all([
          fetch(`/api/quotes/${id}`),
          fetch('/api/clients'),
          fetch('/api/boxes'),
        ]);
        ...
        // Verificar que la cotización esté en borrador
        ...
        setClientId(quoteData.client?.id || '');
        setHasPrinting(quoteData.has_printing);
        setPrintingColors(quoteData.printing_colors || 0);
        setHasExistingPolymer(quoteData.has_existing_polymer);
        setHasDieCut(quoteData.has_die_cut);
        setNotes(quoteData.notes || '');

        // Cargar items
        setItems(quoteData.items.map((item: DBQuoteItem) => ({
          id: item.id,
        ...
        // Cargar diseño existente si tiene impresión
        if (quoteData.has_printing) {
          try {
            const designRes = await fetch(`/api/quotes/${id}/design`);
            if (designRes.ok) {
              const designData = await designRes.json();
              if (designData && designData.file_url) {
                setExistingDesign({
                  name: designData.name || 'Diseño',
                  file_url: designData.file_url,
                });
              }
            }
          } catch {
            // No hay diseño existente
          }
        }
  ...
  function updateItem(itemId: string, field: keyof QuoteItem, value: number | string) {
  ...
  async function removeExistingDesign() {
    if (!existingDesign) return;

    try {
      await fetch(`/api/quotes/${id}/design`, { method: 'DELETE' });
      setExistingDesign(null);
    } catch (error) {
    ...
    try {
      const res = await fetch(`/api/quotes/${id}`, {
        ...
        body: JSON.stringify({
          ...
          notes,
          // Datos calculados
          total_m2: calculation.summary.total_m2,
          price_per_m2: calculation.summary.price_per_m2,
          subtotal: calculation.summary.subtotal,
          printing_cost: calculation.summary.printing_cost,
          shipping_cost: calculation.summary.shipping_cost,
          shipping_notes: calculation.summary.shipping_notes,
          total: calculation.summary.total,
          production_days: calculation.summary.production_days,
        }),
      });
      ...
      // Si hay archivo de diseño nuevo, subirlo
      if (designFile && hasPrinting) {
        ...
        try {
          ...
          if (uploadRes.ok && uploadData.url) {
            ...
            await fetch(`/api/quotes/${id}/design`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Editar Cotizacion</h1>
              ...
              {hasPrinting && (
                <div className="pl-6 space-y-4">
                  <Input
                    ...
                    onChange={(e) => {
                      setPrintingColors(Math.min(3, parseInt(e.target.value) || 0));
                      setCalculation(null);
                    }}
                    min={1}
                    max={3}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => {
                        setHasExistingPolymer(e.target.checked);
                        setCalculation(null);
                      }}
                      className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                  ...
                  <div className="pt-2">
                    ...
                    {/* Botón para cambiar archivo cuando ya hay uno existente */}
                    {existingDesign && !designFile && (
                      <label className="mt-2 inline-flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 cursor-pointer">
              ...
              {!calculation ? (
                <div className="text-center py-4">
                  <p className="text-gray-500 text-sm mb-4">
                    Modifique los datos y haga clic en &quot;Recalcular Cotizacion&quot; para ver el nuevo total
                  </p>
                  <Badge variant="warning">Pendiente de recalcular</Badge>
                </div>
              ) : (
