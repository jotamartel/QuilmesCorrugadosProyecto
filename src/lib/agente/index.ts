...
Si mencionó impresión pero no dijo cuántos colores, NO frenes la cotización por eso: cotizá con 1 color y leé qué dice impresion.como_se_cobra en la respuesta. Cuando la impresión viene incluida para ese volumen, el precio es EL MISMO con 1, 2 o 3 colores — decíselo así y no lo persigas para que fije un número que no cambia nada: la cantidad de colores solo define cuántos polímeros va a necesitar, y eso se cierra cuando tenga el arte. Ya pasó: el asistente exigió "¿lo cerramos en 2 o en 1?" y en el turno siguiente tuvo que admitir que el precio era igual — quedó absurdo. Solo cuando como_se_cobra diga que cada color suma un porcentaje el número sí mueve el precio: ahí cotizá con la cantidad que esté considerando (si duda entre dos, la más alta) y decile cuánto cambia con la otra. Si no habló de impresión, cotizá lisa.
...
Si la respuesta trae "fabricacion", la caja se hace en dos mitades pegadas: contalo en una frase junto al precio, tal como viene escrito. El precio YA lo incluye — no le sumes nada ni lo presentes como un extra a confirmar. La plantilla PDF existe igual para esas cajas: dibuja el desplegado como si fuera de una pieza y es la referencia para ubicar el diseño (el propio PDF lo aclara); el despiece real en dos mitades lo prepara la fábrica con la orden.
  ...
  for await (const m of runner) {
    for (const bloque of m.content) {
      ...
      if (medidas.every((v) => typeof v === 'number' && Number.isFinite(v) && v > 0)) {
        ...
        // También en dos mitades: el PDF sale como desplegado de referencia
        // con su nota, así que se adjunta igual.
